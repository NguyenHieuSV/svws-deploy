...
class YeuCauMua(Base):
    ...
    vuot_du_toan: Mapped[str | None] = mapped_column(String(300), nullable=True)  # mig 119: lý do vượt/ngoài dự toán → chỉ CEO duyệt
...
class KhoanVay(Base):
    ...
    chu_ky_thang: Mapped[int] = mapped_column(Integer, default=1)   # cũ (giữ tương thích)
    chu_ky_ngay: Mapped[int | None] = mapped_column(Integer, nullable=True)   # số ngày mỗi kỳ (mig 73)
...
class CtHoaDonDauVao(Base):
    ...
    don_vi_hang: Mapped[str | None] = mapped_column(String(20), nullable=True)
    don_gia: Mapped[Decimal | None] = mapped_column(Numeric(18, 2), nullable=True)
...
class DichVuKT(Base):
    ...
    khach_ten: Mapped[str | None] = mapped_column(String(150), nullable=True)
    cong_ty: Mapped[str | None] = mapped_column(String(200), nullable=True)
    dien_thoai: Mapped[str | None] = mapped_column(String(40), nullable=True)
