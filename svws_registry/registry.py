...
def ai_execute(payload: dict = Body(...)):
    ...
    if not api_key:
        raise HTTPException(503, "Server chưa cấu hình ANTHROPIC_API_KEY "
                                 "(Render → Environment → thêm biến này)")
    model = os.getenv("ANTHROPIC_MODEL", "claude-sonnet-5")
    # Trần token ĐẦU RA. Đặt bằng mức tối đa model cho phép: Sonnet 5 và Opus 5
    # đều cho tới 128.000, với điều kiện dùng streaming để khỏi vướng timeout
    # HTTP — chỗ này đã stream sẵn nên dùng được hết.
    # BẪY: token SUY NGHĨ (adaptive thinking) TÍNH VÀO trần này. Để 64.000 thì
    # một tool 11 tab vừa nghĩ vừa viết là chạm trần và đứt giữa chừng — đã xảy ra
    # với bản Cụm RO 30 m³/h.
    max_tokens = int(os.getenv("ANTHROPIC_MAX_TOKENS", "128000"))
    ...
    user_msg = (prompt + "\n\nQUAN TRỌNG: Trả về TRỰC TIẾP file HTML hoàn chỉnh "
                ...
                "SÁU ĐIỀU DƯỚI ĐÂY BỊ KIỂM TỰ ĐỘNG NGAY KHI BẠN NỘP FILE. Thiếu bất "
                ...
                "  5. SVWSKT.veAnToan(el, '<tên tab>', function(){…}) bọc MỌI hàm "
                "dựng tab.\n"
                "  6. SVWSVT.xuatCauHinh(design, params, SO.xuat()) ở nút Lưu JSON.\n"
                "Đã có tool nộp lên chạy đẹp, mọi tab đầy đủ, nhưng bỏ qua ba sổ: bảng "
                "thông số ghi 2 cột lọc mà mô hình dựng 1 cột, còn vỏ lọc tinh và bơm "
                "cao áp thì không có trong bản vẽ — nhìn ảnh không ai phát hiện. Đó là "
                "lý do sáu điều trên bị kiểm bằng máy chứ không tin vào mắt.\n\n"
                ...
                "  ★ CÁCH ĐÚNG NHẤT — sinh thẳng từ CÙNG BỘ EQUIP + PIPES đã dùng dựng "
                "3D, chỉ MỘT dòng:\n"
                "    const D = SVWSPID.to({w:1500, h:700, ma:'<mã>', ten:'<tên>', dong2:'<cân bằng lưu lượng>'});\n"
                "    D.heThong(EQUIP, PIPES);          // tự xếp theo thứ tự dòng chảy\n"
                "    D.dungCuTuIO(IO.danhSach());      // BẦU ĐO LẤY TỪ BẢNG I/O tủ điện\n"
                "    D.dungCu('T-101','LIT-101');      // chỉ dùng khi cần thêm cái lẻ\n"
                "  D.dungCuTuIO() đọc thẳng bảng I/O của SVWSDIEN.plc: mỗi kênh DI/AI là "
                "một dụng cụ đo thật ngoài hiện trường nên thành một bầu đo trên sơ đồ "
                "(DO/AO là lệnh điều khiển, tự bỏ qua). Ghép theo SỐ VÒNG LẶP trong tag "
                "(LIT-101 → thiết bị có số 101) và theo chữ cái đầu ISA (L→bồn, P→bơm/cụm "
                "màng, A→cụm màng/bồn); khai rõ tb:'<tag thiết bị>' trong kênh I/O khi "
                "muốn chỉ định thẳng. Nhờ vậy P&ID và bảng I/O KHÔNG THỂ LỆCH NHAU: thêm "
                "một đầu đo vào PLC là bầu đo hiện lên sơ đồ.\n"
                "  kiemTra() đối chiếu hai chiều: kênh I/O không gắn được vào thiết bị "
                "nào (sai số vòng lặp), và ngược lại — bồn nào chưa có đo mức, cụm màng "
                "nào chưa có đo áp suất hoặc độ dẫn thì báo lỗi, vì vận hành mù.\n"
                "  Vì vậy hãy dựng tab 7 (bảng I/O) TRƯỚC hoặc cùng lúc với tab 2.\n"
                "    el.innerHTML = D.ve();  console.log(D.kiemTra());\n"
                "  Thư viện coi mỗi thiết bị là một NÚT có cổng vào/ra, mỗi đường ống là "
                "một CẠNH; tự xếp cột theo thứ tự công nghệ (sắp xếp tô-pô), tự tách "
                "tuyến HỒI LƯU vẽ vòng xuống dưới, tự nối đúng tuyến với màu theo lưu "
                "chất và nhãn DN, tự đẩy nhãn khỏi đè, tự nới khổ giấy. Nhờ vậy P&ID "
                "LUÔN khớp bản vẽ 3D — thêm thiết bị vào EQUIP là P&ID có ngay, không ai "
                "phải soát chồng lấn bằng mắt.\n"
                "  kiemTra() lúc này kiểm cả TÍNH LIỀN LẠC: thiết bị không nối vào tuyến "
                "nào, thiết bị chỉ có đường vào mà không có đường ra, đường ống trỏ tới "
                "thiết bị không tồn tại, khai ngược chiều from/to.\n"
                "  Chỉ dùng cách gọi tay từng hàng dưới đây khi cần vẽ thêm mạch phụ "
                "(CIP, khí nén) không nằm trong PIPES:\n"
                    ...
                    if etype == "content_block_delta":
                        ...
                        if stop == "max_tokens":
                            them = ("Đây đã là mức tối đa model cho phép. "
                                    ...
                                    "Tăng ANTHROPIC_MAX_TOKENS trên Render (tối đa 128000). ")
                            yield (f"\n[SVWS-LỖI] File bị cắt vì chạm trần {max_tokens:,} token "
                                   f"(đã dùng {used}). " + them +
                                   "Cách giảm lượng chữ AI phải viết: bớt tab chưa cần cho "
                                   "lần này, hoặc soạn ba sổ trước rồi mới sinh tool (có sổ "
                                   "sẵn thì AI không phải nghĩ ra danh sách thiết bị nữa), "
                                   "hoặc đặt ANTHROPIC_EFFORT=low để bớt phần suy nghĩ.")
                            return
...
def ai_baso(payload: dict = Body(...)):
    ...
    sys_msg = (
        ...
        "xa (điểm xả / hố ga thoát nước — nơi nước bỏ đi rời khỏi hệ) · "
        "vessel (cột lọc áp lực) · filter · mixedbed · cartridge (vỏ lọc tinh, bag "
        "filter) · phanung (thiết bị phản ứng: điện hoá EC, oxy hoá, tiếp xúc — "
        "KHÔNG phải cột lọc, không có lớp vật liệu, không rửa ngược) · "
        "pump · dosing (cụm châm hoá chất) · blower (máy thổi khí, quạt thông "
        "gió) · roskid · edi · uv · panel\n"
        ...
        "   kW, kieuDien (DOL|VFD|KHAC) · cho thiết bị có động cơ (pump, dosing, "
        "blower). BỂ CÓ CÁNH KHUẤY (bể phản ứng, bể trung hoà, bể tạo bông) "
        "vẫn khai loai=tank và ghi kW của mô-tơ khuấy — đó là cách khai ĐÚNG. "
        "Tải ăn điện mà không qua khởi động từ kiểu động cơ (bộ chỉnh lưu DC "
        "của cụm điện hoá, chấn lưu đèn UV, điện trở sưởi) thì khai kW và đặt "
        "kieuDien=KHAC\n"
        ...
        "và P-101B.\n"
        "- ĐIỂM CHÂM: tuyến hoá chất được phép nối vào MỘT TUYẾN ỐNG thay vì vào "
        "một thiết bị — ô 'den' điền MÃ TUYẾN (ví dụ DOS-101 châm vào L-05). Đây "
        "là cách khai đúng vị trí thật của điểm châm; chỉ dùng khi dv là 'chem' "
        "và mã tuyến đó có thật trong sổ.\n"
        "- NƯỚC BỎ ĐI PHẢI CÓ CHỖ ĐI. Cột lọc rửa ngược (vessel, filter, mixedbed), "
        "cụm màng xả cô đặc và cụm EDI đều BẮT BUỘC có một tuyến dv=waste đi "
        "ra. Đầu đến là một dòng loai=xa (hố ga), HOẶC một thiết bị nếu thiết "
        "kế cho nước ấy quay lại xử lý (ví dụ nước rửa lọc về bể điều hoà). "
        "Cấm bỏ trống ô den. Được phép khai NHIỀU điểm xả.\n"
        "- Thiết bị lắp sẵn trên khung của cụm khác (vỏ lọc bảo vệ màng và bơm "
        "cao áp nằm trên khung RO) khai thêm trường tren=<tag cụm> — chúng vẫn "
        "là dòng riêng nhưng không chiếm chân đế riêng trên mặt bằng.\n"
